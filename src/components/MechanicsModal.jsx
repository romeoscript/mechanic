...
import { MapPin, Phone, Star, MessageCircle, Clock, Wrench, ChevronRight, X, Send } from 'lucide-react';

const MechanicsModal = ({ showModal, setShowModal, mechanics, isLoadingMechanics }) => {
    const [selectedMechanic, setSelectedMechanic] = useState(null);
    const [showChat, setShowChat] = useState(false);
    const [message, setMessage] = useState('');
    const [chatMessages, setChatMessages] = useState({});
  
    const handleSendMessage = useCallback((mechanicId) => {
      ...
      setChatMessages(prev => ({
        ...
        [mechanicId]: [
          ...(prev[mechanicId] || []),
          { text: message, sender: 'user', time: new Date() }
        ]
      }));
    ...
    const ChatInterface = useCallback(({ mechanic }) => (
        <div className="absolute inset-0 bg-white z-10 flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {(chatMessages[mechanic.id] || []).map((msg, idx) => (
          ...
          <div className="border-t p-4">
            <div className="flex gap-2">
              <input
                ...
                onKeyPress={(e) => e.key === 'Enter' && handleSendMessage(mechanic.id)}
              />
              <button
      ...
      ), [message, chatMessages, setShowChat, handleSendMessage]);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[80vh] overflow-hidden relative">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-gray-500">Find trusted professionals in your area</p>
            </div>
            <button 
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded">
                          ...
                          <span className="font-semibold">{mechanic.rating || 'N/A'}</span>
                        </div>
                      </div>
                      ...
                      <div className="mt-4 flex items-center gap-3">
                        ...
                        <span className="text-sm text-gray-500">{mechanic.distance.toFixed(1)} km away</span>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {showChat && selectedMechanic && (
          <ChatInterface mechanic={selectedMechanic} />
        )}
      </div>
    </div>
  );
